...
public class IndexedTextRenderer(IRenderer2 renderer, IPaletteSource paletteSource) : IIndexedTextRenderer
{
        ...
        TextSpacing spacing = TextSpacing.Normal, byte? outlineColor = null, int lineSpacing = 0, TextRenderingContext context = null)
    {
        ...
        renderer.TextRenderer.DrawText(font, text, position, align, 1, paletteSource.Palette[color], spacing, outlineClr, lineSpacing, context);;
    }
    ...
    {
        RgbaColor? outlineClr = outlineColor.HasValue ? paletteSource.Palette[outlineColor.Value] : null;
        renderer.TextRenderer.DrawText(font, text, position, align, 1, paletteSource.Palette[color], spacing, paragraphSpacing, outlineClr, lineSpacing, context);;
    }
}