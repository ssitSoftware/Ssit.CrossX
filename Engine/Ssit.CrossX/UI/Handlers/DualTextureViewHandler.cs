...
public class DualTextureViewHandler : BackgroundHandler<DualTextureView>
{
    ...
    protected virtual RgbaColor? Color(IRenderer2 renderer) => AttachedView.Color?.GetColor(PaletteSource, renderer, _colorSource);
    protected virtual RgbaColor? OutlineColor(IRenderer2 renderer) => AttachedView.OutlineColor?.GetColor(PaletteSource, renderer, _colorSource);

    public DualTextureViewHandler(CreateHandlerParameters parameters, IIoCContainer container, IPaletteSource paletteSource = null) : base(parameters, paletteSource)
    {
        if (AttachedView.Source is null)
        {
            throw new InvalidOperationException("The dual texture view is missing a Source.");
        }
    ...
    public override void CalculateSize(out Length width, out Length height)
    {
        ...
        if (width.IsAuto || height.IsAuto)
        {
            ...
            var size = AttachedView.Source?.SourceRect?.Size ?? texture?.Resource.Texture.Size;
    ...
    protected override void OnDraw(IRenderer2 renderer)
    {
        ...
        CalculateTargetRects(texture.Resource.Texture, source, out var targetRect, out var sourceRect);

        var color = Color(renderer);
        var outlineColor = OutlineColor(renderer);

        renderer.SpriteRenderer.Draw(texture.Resource.Texture, targetRect, sourceRect, null, color, AttachedView?.Transform ?? ImageTransform.None);
        renderer.SpriteRenderer.Draw(texture.Resource.Outline, targetRect, sourceRect, null, outlineColor, AttachedView?.Transform ?? ImageTransform.None);
    }
    ...
    private void CalculateTargetRects(ITexture texture, Rectangle? source, out RectangleF targetRect, out RectangleF sourceRect)
    {
        ...
        switch (AttachedView.Scaling)
        {
            case ImageScalingMode.AspectFit:
            {
                ...
                targetSize = new SizeF(size.Width * scale, size.Height * scale);
            }
            break;
